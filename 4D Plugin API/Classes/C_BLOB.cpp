...
void FOURD::C_BLOB::fromParamAtIndex(PA_PluginParameters params, uint32_t index)
{
	this->_CBytes->fromParamAtIndex(params, index);
}

void FOURD::C_BLOB::fromParamAtIndex(PackagePtr pParams, uint32_t index)
{
	this->_CBytes->fromParamAtIndex(pParams, index);
}

void FOURD::C_BLOB::toParamAtIndex(PackagePtr pParams, uint32_t index)
{
	this->_CBytes->toParamAtIndex(pParams, index);	
}

void FOURD::C_BLOB::setReturn(sLONG_PTR *pResult)
{
	this->_CBytes->setReturn(pResult);
}

void FOURD::C_BLOB::setBytes(const uint8_t *bytes, unsigned int len)
{
	this->_CBytes->setBytes(bytes, len);
}

void FOURD::C_BLOB::addBytes(const uint8_t *bytes, unsigned int len)
{
	this->_CBytes->addBytes(bytes, len);
}

const uint8_t *FOURD::C_BLOB::getBytesPtr()
{
	return this->_CBytes->getBytesPtr();
}

uint32_t FOURD::C_BLOB::getBytesLength()
{
	return this->_CBytes->getBytesLength();	
}

const uint8_t *FOURD::C_BLOB::getBytesPtrForSize(uint32_t *size)
{
	return this->_CBytes->getBytesPtrForSize(size);		
}

void FOURD::C_BLOB::fromHexText(C_TEXT *hex)
{
	this->_CBytes->fromHexText(hex);	
}

void FOURD::C_BLOB::fromB64Text(C_TEXT *b64)
{
	this->_CBytes->fromB64Text(b64);	
}

void FOURD::C_BLOB::toHexText(C_TEXT *hex)
{
	this->_CBytes->toHexText(hex);	
}

void FOURD::C_BLOB::toB64Text(C_TEXT *b64, bool fold)
{
	this->_CBytes->toB64Text(b64, fold);
}


FOURD::C_BLOB::C_BLOB() : _CBytes(new CBytes)
{
